...
func (base *Controller) VerifyEmail(c *gin.Context) {
	...
	code, err := verification.VerifyEmailService(base.ExtReq, base.Logger, req, base.Db)
	if err != nil {
		rd := utility.BuildErrorResponse(code, "error", err.Error(), err, nil)
		c.JSON(code, rd)
		return
	...
	rd := utility.BuildSuccessResponse(http.StatusOK, "E-mail verification successful.", nil)
	c.JSON(http.StatusOK, rd)
